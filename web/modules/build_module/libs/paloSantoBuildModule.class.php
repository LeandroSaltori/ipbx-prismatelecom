...
class paloSantoBuildModule {
    ...
    private function Query_Issabel_Version()
    {
        $query = "SELECT value FROM settings WHERE key='issabel_version_release'";
        ...
        $sModuleType, $fieldList)
    {
        ...
        $xml_modulespec->addChild('issabelversion', $this->Query_Issabel_Version());
        ...
        $output = $retval = NULL;
        exec('/usr/bin/issabel-helper develbuilder --createmodule '.escapeshellarg($sTempFile).' 2>&1',
            $output, $retval);
        unlink($sTempFile);
        if ($retval != 0) {
